...
When the **Null Deviance** is really small, it means that the Null Model explains the data pretty well. The same goes for **Residual Deviance**. To best interpret our models, we will compare the `Residual deviance.`
...
c_val = qt(0.025, df = 26, lower.tail = FALSE) %>% 
  print()
...
ci_lower_murder <- round(murder_model$coefficients[2,'Estimate'] - c_val*murder_model$coefficients[2,'Std. Error'], 2) %>% 
  print()

ci_upper_murder <- round(murder_model$coefficients[2,'Estimate'] + c_val*murder_model$coefficients[2,'Std. Error'], 2) %>% 
  print()
